...
export class TabBar extends Component {
  ...
  render() {
    ...
    let lis = this.tabs.map((tab, i) => {
      ...
      let inner = (
        <a data-mui-toggle="tab" data-mui-controls={paneid}>
          {label}
        </a>
      );
      ...
      let act =
        i == this.active ? (
          <li class="mui--is-active">{inner}</li>
        ) : (
          <li>{inner}</li>
        );
      return act;
  ...
  rendered() {
    ...
    if (!this.listenInitialized) {
      ...
      tabsEl.addEventListener("mui.tabs.showend", ev => {
        ...
        evv.currTabIndex = tabsEl.dispatchEvent(evv);
      });
    }
  }
...
export class MuiButton extends Component {
  ...
  render() {
    var btnclasses =
      "mui-btn" +
      (this.primary ? " mui-btn--primary" : "") +
      (this.danger ? " mui-btn--danger" : "") +
      (this.accent ? " mui-btn--accent" : "") +
      (this.flat ? " mui-btn--flat" : "") +
      (this.raised ? " mui-btn--raised" : "") +
      (this.fab ? " mui-btn--fab" : "") +
      (this.small ? " mui-btn--small" : "") +
      (this.large ? " mui-btn--large" : "");
...
export class MuiContainer extends Component {
  ...
  render() {
    ...
    return (
      <div>
        <style>{st}</style>
        <div class={this.fluid ? "mui-container-fluid" : "mui-container"}>
          <slot />
        </div>
      </div>
    );
...
export class MuiTextField extends Component {
  ...
  render() {
    ...
    if (this.label && !this.fixed && !this.value)
      fieldClass += " mui-textfield--float-label";
    let props = this.textarea
      ? {}
      : { type: this.type ? this.type : "text", value: this.value };
    ...
    var field = this.textarea ? (
      <textarea {...props}>{this.value}</textarea>
    ) : (
      <input {...props} />
    );
      ...
      return (
        <div>
          ...
          <div class={fieldClass}>
            {field}
            <label {...labelStyle}>{this.label}</label>
            <div class="mui-error">{this.errorMessage}</div>
          </div>
        </div>
      );
    }
...
export class MuiCheckBox extends Component {
  ...
  render() {
    ...
    if (this.disabled) props.disabled = true;
    let field = <input type="checkbox" {...props} />;
    return (
      <div>
